import {
  ...
  realTimeEvent,
  startServer,
} from "soquetic"; // importo las funciones de soquetic ( suscribe get event devuelve datos, susctibe post event los recibe)
...
function numero(data) {
  // data es el objeto que manda el frontend. tiene la informacion de los ids y si es true or false ( agregarlos o no) // booleano es un dato que tiene 2 valores, true y false.

  if (data.enColección == true) {
    // en coleccion que indica true si la obra debe agregarse o false si la obra no debe agregarse.
    let contenido = JSON.parse(
      fs.readFileSync("../data/idcoleccion.json", "utf-8")
    ); // leo el archivo de los ids en texto, lo convierto en array y lo guardo en la variable contenido.
...
function devolverColeccion() {
  ...
  for (let i = 0; i < obras.length; i++) {
    // mientras que el for recorre el array en bucle, i aumenta uno por cada obra que pase.
    if (ids.includes(obras[i].id)) {
      // si el id de la obra se encuentra en variable ids que contiene el array con los ids se agrega a la variable obrasencoleccion.
